    ...
    function PtiFlowGeneratorNode(config) {
        ...
        function generateNodeRedFlow(dashboardTabName, dashboardGroupName, inputFields, enableBasicFlow, enableBarcodeEvent, enableResultEvent, enableBoardCodeReturn) {
            ...
            nodes.push({  
                ...
                z: flowId,
                name: "",
                filename: dashboardTabName+"_"+dashboardGroupName,
                filenameType: "str",
                appendNewline: true,
                createDir: true,
                overwriteFile: "true",
...
boards 所有板的测试结果 
//（用这个判断是否全部上成功）
outputFlag 出站循环上传flag 
//用来存储过站返回异常的内容
outputMessage 出站循环message
//用来存储所有返回的内容 
outputReturnData 出站返回所有内容 
            `,
                x: 60,
                y: 500,
                wires: []
...
return msg;`,
    ...
    "Fixture":"{{payload.fixture}}"
            ...
            const httpReturnJsonDebugId = generateFixedId();
...
return msg;`,
    ...
    "InfoText": "扫码接口请求超时，请检查服务器是否联通正常。{{{payload}}}"
            ...
            barcodeNodes.push({
                ...
                z: flowId,
                name: "",
                filename: "D:\\PTI-MES\\json\\扫码返回数据.json",
                filenameType: "str",
                appendNewline: true,
                createDir: true,
                overwriteFile: "true",
            ...
            barcodeNodes.push({
                ...
                x: 1810,
                y: 1960,
                wires: [[statusSwitchId,httpReturnJsonDebugId]]
            });
    ...
    "InfoText": "{{payload.ResMsg}}"
        ...
        function generateResultEventNodes(flowId) {
            ...
            resultNodes.push({ 
                ...
                z: flowId,
                name: "出站参考",
                info: `var snList = []; boards.forEach(function (element, index) {
    if (element.data.TestStat != "Skip") {
        ...
        if (element.data.componentData != undefined) {
            element.data.componentData.forEach(function (data) {
                if (data.Result != "Skip") { }});}
        if (element.data.functionData != undefined) {
            element.data.functionData.forEach(function (data) { });}}}snList.push();});
// @ts-ignore
global.set("snList", snList);return msg;`,
                x: 980,
                y: 2600,
                wires: []
            });
...
var outputReturnData = []
// @ts-ignore
global.set("outputReturnData", outputReturnData);
// @ts-ignore
var userName = global.get("userName");
var snList = [];
...
return msg;`,
            ...
            resultNodes.push({
                ...
                x: 1210,
                y: 2960,
                wires: [[uploadDebugId, uploadFileId,changeOutputUrlId]]
            });
            ...
            resultNodes.push({
                ...
                z: flowId,
                name: "",
                filename: "D:\\PTI-MES\\json\\过站发送模板.json",
                filenameType: "str",
                appendNewline: true,
                createDir: true,
                overwriteFile: "true",
...
"InfoText": "出站接口请求超时，请检查服务器是否联通正常。{{{payload}}}"
            ...
            resultNodes.push({
                ...
                z: flowId,
                name: "",
                filename: "D:\\PTI-MES\\json\\过站返回数据.json",
                filenameType: "str",
                appendNewline: true,
                createDir: true,
                overwriteFile: "true",
            ...
            // 存储并记录outputReturnData函数
            const outputDataDebugId = generateFixedId();
            resultNodes.push({
                id: storeOutputDataId,
                type: "function",
                z: flowId,
                name: "存储并记录outputReturnData",
                func: `// @ts-ignore
var outputIndex =global.get("outputIndex");
// @ts-ignore
var outputReturnData = global.get("outputReturnData");
var data=msg.payload;
outputReturnData[outputIndex] = data;
// @ts-ignore
global.set("outputReturnData", outputReturnData);
// @ts-ignore
var BoardCount = global.get("BoardCount");

msg.payload=outputReturnData;
return msg;`,
                outputs: 1,
                timeout: 0,
                noerr: 0,
            ...
            // outputReturnData调试节点
            resultNodes.push({
                id: outputDataDebugId,
                type: "debug",
                z: flowId,
                name: "outputReturnData",
                active: true,
                tosidebar: true,
                console: true,
                tostatus: false,
...
var outputFlag = true;
var outputMesg = "";
outputReturnData.forEach(function(element) {
...
msg.payload = {
    ...
    "data": outputReturnData // 根据需要调整返回的数据结构
};
